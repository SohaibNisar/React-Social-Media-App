...
let FullWidthTabs = (props) => {
  ...
  let {
    ...
    user: { authenticated },
    staticUser: {
      credentials: { friends, staticUserHandle },
      posts,
      credentials,
    },
  } = props;

  const listButtonBreak = useMediaQuery(
    "(max-width:670px) and (min-width:600px),(max-width:430px)"
  );
  const tabsDoubleButtonMediaQuery = useMediaQuery(
    "(max-width:716px) and (min-width:600px),(max-width:494px)"
  );
  ...
  if (friends) {
    if (authenticated) {
      friends = friends.filter(
        (friend) =>
          friend.userHandle !== user.credentials.userHandle &&
          friend.userHandle !== staticUserHandle
      );
    } else {
      friends = friends.filter(
        (friend) => friend.userHandle !== staticUserHandle
      );
    }
  } else {
    friends = [];
  ...
  return (
    <Root>
          ...
          <Toolbar className={classes.toolBar} variant="dense" disableGutters>
            ...
            <div className={classes.button}>
              {user.credentials.userHandle === credentials.userHandle ? (
                <EditProfile />
              ) : !user.credentials.friends.some(
                  (friend) => friend.userHandle === credentials.userHandle
                ) ? (
                <AddFriend
                  verticle={true}
                  friendUserHandle={credentials.userHandle}
                  confirmBtnStyle={
      ...
      <SwipeableViews axis="x" index={value} onChangeIndex={handleChangeIndex}>
        <>
          {value === 0 && posts.length > 0 ? (
            posts.map((post) => (
              <Post
                post={post}
                key={post.id}
                openPostDialog={
                  params.postId && params.postId === post.id ? true : false
                }
              />
            ))
          ) : (
        ...
        <>
          {value === 2 && (
            <Paper>
              {friends.length > 0 ? (
                <FriendsList
                  friendsToList={friends}
                  buttonONBottom={listButtonBreak}
                />
              ) : (
                <Typography
                  align="center"

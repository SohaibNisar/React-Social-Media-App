...
class PostDialog extends Component {
  ...
  render() {
    ...
    let {
      singlePost,
      user: { likes, credentials, authenticated },
      UI,
    } = this.props;

    let liked, deleteButton;

    if (!UI.loading && credentials) {
      liked = likes && likes.some((like) => like.postId === singlePost.id);
      deleteButton =
        singlePost && credentials
          ? singlePost.userHandle === credentials.userHandle
          : false;
    }

    return (
      <>
        {UI.loading || !singlePost ? (
          ...
          <Root variant="outlined" sx={{ border: "none" }}>
            <CardHeader
              avatar={
                <Avatar
                  alt={singlePost.userHandle}
                  src={singlePost.profilePicture}
                />
              }
              action={
                authenticated && deleteButton ? (
                  <DeletePostButton postId={singlePost.id} />
                ) : null
              }
              title={
                <MuiLink
